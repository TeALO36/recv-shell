...
#include <stdio.h>
#include <stdlib.h>
#include <zip.h>     // Inclure la bibliothèque libzip
...
void decompress_zip(const char *zip_path, const char *extract_path) {
    int err = 0;
    zip_t *zip_archive = zip_open(zip_path, ZIP_RDONLY, &err);

    if (zip_archive == NULL) {
        const char *zip_error = zip_strerror(zip_archive);
        printf("Impossible d'ouvrir le fichier ZIP : %s\n", zip_error);
        return;
    }

    zip_uint64_t num_files = zip_get_num_entries(zip_archive, 0);
    for (zip_uint64_t i = 0; i < num_files; i++) {
        const char *file_name = zip_get_name(zip_archive, i, 0);
        if (file_name == NULL) {
            printf("Erreur lors de la lecture du nom de fichier dans le ZIP\n");
            continue;
        }

        // Créer le chemin de sortie complet
        char extract_file_path[MAX_PATH];
        snprintf(extract_file_path, MAX_PATH, "%s\\%s", extract_path, file_name);
        ...
        // Extraire le fichier
        zip_file_t *zip_file = zip_fopen(zip_archive, file_name, 0);
        if (zip_file == NULL) {
            printf("Erreur lors de l'ouverture du fichier dans le ZIP : %s\n", file_name);
            continue;
        }

        FILE *output_file = fopen(extract_file_path, "wb");
        if (output_file == NULL) {
